...
class ConvAttention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, num_layers=6,
                 num_channels=64, kernel_size=5,
        ...
        self.conv_layers = nn.ModuleList(
            [nn.Conv2d(in_channels=num_attention_heads, out_channels=num_channels,
    ...
    def forward(self, queries, keys, values, attention_mask):
        ...
        for conv in self.conv_layers:
            if attention_mask is not None:
                attention_scores = attention_scores + attention_mask.to(key.dtype)
            attention_scores = torch.softmax(attention_scores, dim=-1)
            attention_scores = conv(attention_scores)
