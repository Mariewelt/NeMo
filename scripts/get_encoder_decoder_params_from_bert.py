...
decoder_starts_from = int(len(bert_keys[5:]) / 2)
...
for i in range(5):
    encoder_from_bert[decoder_keys[i]] = bert_keys[i]
    decoder_from_bert[decoder_keys[i]] = bert_keys[i]

for i in range(5, decoder_starts_from):
    encoder_from_bert[nemo_bert_mapping[bert_keys[i]]] = bert_keys[i]

cur_layer = 0
num_layers = int(bert_keys[decoder_starts_from].split(".")[3])
for i in range(decoder_starts_from, len(bert_keys)):
    key = nemo_bert_mapping[bert_keys[i]]
    if ("pooler" not in key) and ("cls" not in key):
        tmp = key.split(".")
        cur_layer = int(tmp[2]) - num_layers
        if "first" in key:
            key_first = ".".join(
...
new_encoder_weights = {}
for key in encoder_from_bert.keys():
    new_encoder_weights[key] = weights_bert[encoder_from_bert[key]]
...
new_encoder_weights['embedding_layer.token_embedding.weight'] = tmp
new_decoder_weights['embedding_layer.token_embedding.weight'] = tmp
torch.save(new_encoder_weights, args.save_to + args.model_name + "_encoder.pt")
